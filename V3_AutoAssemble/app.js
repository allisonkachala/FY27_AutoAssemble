...
const PREVIEW_IMAGES = {
  success: ASSETS.previewPurple,
  warning: ASSETS.previewPurple,
  error: ASSETS.previewPurple,
};

/** Figma node 93:40593 on artboard 93:39982 */
const SUGGESTIONS_POPOVER = { left: 1419, top: 193, width: 284, height: 226 };

/** Figma node 135:61901 @ artboard 135:60410 */
const PLACEMENT_SETTINGS_POPOVER = { left: 1616, top: 268, width: 284, height: 278 };
...
/** Dialog status badges — Figma nodes 163:93599, 163:91689, 163:93455, 163:93743 */
const PLACEMENT_SETTINGS_BADGES = {
  1: { text: 'Best Match', variant: 'success' },
  2: { text: 'Highly Similar Match', variant: 'highly-similar' },
  3: { text: 'Similar Match', variant: 'similar' },
  4: {
    text: 'Similar Match',
    variant: 'similar',
    warning: 'Possible interference with other component in assembly.',
  },
};

const PLACEMENT_SETTINGS_BADGE_VARIANTS = [
  'placement-settings-popover__badge--success',
  'placement-settings-popover__badge--highly-similar',
  'placement-settings-popover__badge--similar',
];
...
const EXPERIENCE_LABELS = {
  'atomic-constraints': 'Version 3',
  'joints-constraints': 'Version 4',
...
const state = {
  activeTab: 'data',
  prototypeExperience: 'atomic-constraints',
  prototypeExperienceMenuOpen: false,
  ...
  placementBarInsertMenuOpen: false,
};
...
function init() {
  els = {
    ...
    prototypeExperienceBtn: document.getElementById('prototype-experience-btn'),
    prototypeExperienceMenu: document.getElementById('prototype-experience-menu'),
    prototypeExperienceLabel: document.getElementById('prototype-experience-label'),
    prototypeFrame: document.querySelector('.prototype-frame'),
    prototypeV3: document.getElementById('prototype-v3'),
    prototypeV4: document.getElementById('prototype-v4'),
    prototypeV4Frame: document.getElementById('prototype-v4-frame'),
    prototypeV51: document.getElementById('prototype-v5-1'),
    prototypeV51Frame: document.getElementById('prototype-v5-1-frame'),
    prototypeV52: document.getElementById('prototype-v5-2'),
    prototypeV52Frame: document.getElementById('prototype-v5-2-frame'),
    prototypeV6: document.getElementById('prototype-v6'),
    prototypeV6Frame: document.getElementById('prototype-v6-frame'),
    prototypeV7: document.getElementById('prototype-v7'),
    prototypeV7Frame: document.getElementById('prototype-v7-frame'),
    prototypeV8: document.getElementById('prototype-v8'),
    prototypeV8Frame: document.getElementById('prototype-v8-frame'),
    prototypeV9: document.getElementById('prototype-v9'),
    prototypeV9Frame: document.getElementById('prototype-v9-frame'),
    ...
    placementBarPreviewAll: document.getElementById('placement-bar-preview-all'),
    placementBarPrev: document.getElementById('placement-bar-prev'),
    placementBarNext: document.getElementById('placement-bar-next'),
    placementBarPageLabel: document.getElementById('placement-bar-page-label'),
    placementBarInsert: document.getElementById('placement-bar-insert'),
    ...
    placementBarSettingsBtn: document.getElementById('placement-bar-settings-btn'),
    ...
    placementSettingsPageLabel: document.getElementById('placement-settings-page-label'),
    ...
    placementSettingsBadge: document.getElementById('placement-settings-badge'),
...
function bindEvents() {
  ...
  els.prototypeExperienceBtn?.addEventListener('click', (e) => {
    e.stopPropagation();
    state.prototypeExperienceMenuOpen = !state.prototypeExperienceMenuOpen;
    els.prototypeExperienceMenu.hidden = !state.prototypeExperienceMenuOpen;
    els.prototypeExperienceBtn.setAttribute('aria-expanded', String(state.prototypeExperienceMenuOpen));
  });

  els.prototypeExperienceMenu?.addEventListener('click', (e) => {
    e.stopPropagation();
    const item = e.target.closest('[data-experience]');
    if (!item) return;
    setPrototypeExperience(item.dataset.experience);
    state.prototypeExperienceMenuOpen = false;
    els.prototypeExperienceMenu.hidden = true;
    els.prototypeExperienceBtn.setAttribute('aria-expanded', 'false');
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    if (state.prototypeExperienceMenuOpen) {
      state.prototypeExperienceMenuOpen = false;
      if (els.prototypeExperienceMenu) els.prototypeExperienceMenu.hidden = true;
      if (els.prototypeExperienceBtn) els.prototypeExperienceBtn.setAttribute('aria-expanded', 'false');
    }
    if (state.placementSettingsOpen
      && !e.target.closest('.placement-settings-popover, .marker-hover-toolbar, .placement-bar')) {
    ...
    if (state.placementBarInsertMenuOpen
      && !e.target.closest('.placement-bar__insert-group, .placement-bar__menu')) {
...
function closeVersion3TransientUi() {
  if (state.placementSettingsOpen) {
    closePlacementSettingsPopover();
  }
  closePlacementSettingsInsertMenu();
  ...
  if (els.uploadModal?.classList.contains('is-open')) closeModal('upload');
  if (els.folderModal?.classList.contains('is-open')) closeModal('folder');
}

function setPrototypeExperience(experienceId) {
  if (!EXPERIENCE_LABELS[experienceId]) return;
  state.prototypeExperience = experienceId;

  if (els.prototypeExperienceLabel) {
    els.prototypeExperienceLabel.textContent = EXPERIENCE_LABELS[experienceId];
  }
  els.prototypeExperienceMenu?.querySelectorAll('[data-experience]').forEach((item) => {
    item.classList.toggle('is-selected', item.dataset.experience === experienceId);
  });

  if (els.prototypeV3) {
    els.prototypeV3.hidden = experienceId !== 'atomic-constraints';
  }
  if (els.prototypeV4) {
    els.prototypeV4.hidden = experienceId !== 'joints-constraints';
  }
  if (els.prototypeV51) {
    els.prototypeV51.hidden = experienceId !== 'version-5-1';
  }
  if (els.prototypeV52) {
    els.prototypeV52.hidden = experienceId !== 'version-5-2';
  }
  if (els.prototypeV6) {
    els.prototypeV6.hidden = experienceId !== 'version-6';
  }
  if (els.prototypeV7) {
    els.prototypeV7.hidden = experienceId !== 'version-7';
  }
  if (els.prototypeV8) {
    els.prototypeV8.hidden = experienceId !== 'version-8';
  }
  if (els.prototypeV9) {
    els.prototypeV9.hidden = experienceId !== 'version-9';
  }

  if (experienceId !== 'atomic-constraints') {
    closeVersion3TransientUi();
  }
}
...
function startPlacement(fileId, artboardX, artboardY) {
  ...
  if (fileId === COMPONENT_2_ID) {
    ...
    if (els.placementBarPreviewAll) {
      setPlacementBarCheckboxChecked(els.placementBarPreviewAll, true);
    }
  }
...
function onPlacementClick(e) {
  if (!state.placement.active) return;
  if (e.target.closest('.context-menu, .modal-overlay, .data-panel, .top-nav, .suggestions-popover, .placement-settings-popover, .placement-bar, .component-overlay, .component-marker, .component-marker-wrap, .marker-hover-toolbar, .placement-toolbar')) return;
...
function shouldShowAllPlacementPreviews() {
  return shouldShowPlacementBar() && state.previewAllPlacements;
}
...
function updatePlacementBar() {
  ...
  const current = state.placementBarIndex + 1;
  const selectedCount = state.selectedMarkerNums.length;

  if (els.placementBarPageLabel) {
    els.placementBarPageLabel.textContent = selectedCount > 0
      ? `${selectedCount} of ${total} selected`
      : `${current} of ${total}`;
  }
  if (els.placementBarPrev) {
    els.placementBarPrev.classList.toggle('is-disabled', state.placementBarIndex <= 0);
  }
  if (els.placementBarNext) {
    els.placementBarNext.classList.toggle('is-disabled', state.placementBarIndex >= total - 1);
  }
...
function closePlacementBarInsertMenu() {
  if (!state.placementBarInsertMenuOpen) return;
  state.placementBarInsertMenuOpen = false;
...
function togglePlacementBarInsertMenu() {
  state.placementBarInsertMenuOpen = !state.placementBarInsertMenuOpen;
  if (els.placementBarInsertMenu) {
    els.placementBarInsertMenu.hidden = !state.placementBarInsertMenuOpen;
  }
  if (els.placementBarInsertSplit) {
    els.placementBarInsertSplit.setAttribute(
      'aria-expanded',
      state.placementBarInsertMenuOpen ? 'true' : 'false',
    );
  }
  if (state.placementBarInsertMenuOpen) {
    updatePlacementInsertMenus();
...
function insertFromPlacementBar(mode) {
  closePlacementBarInsertMenu();
...
function bindPlacementBarEvents() {
  ...
  els.placementBarPreviewAll?.addEventListener('click', (e) => {
    e.stopPropagation();
    const checked = !els.placementBarPreviewAll.classList.contains('is-checked');
    state.previewAllPlacements = checked;
    setPlacementBarCheckboxChecked(els.placementBarPreviewAll, checked);
    if (!checked) {
      const nums = getSelectedPlacementMarkerNums();
      if (nums.length === 1) {
        state.hoveredMarker = nums[0];
      } else if (nums.length === 0) {
        state.hoveredMarker = getFocusedPlacementMarkerNum();
      }
    }
    refreshPlacementOverlay();
    updatePlacingGhost();
  ...
  els.placementBarPrev?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (state.placementBarIndex <= 0) return;
    setPlacementBarIndex(state.placementBarIndex - 1);
  });

  els.placementBarNext?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (state.placementBarIndex >= PLACEMENT_BAR_MARKERS.length - 1) return;
    setPlacementBarIndex(state.placementBarIndex + 1);
  });

  els.placementBarInsert?.addEventListener('click', (e) => {
    e.stopPropagation();
    closePlacementBarInsertMenu();
    confirmPlacement();
  });
  ...
  els.placementBarInsertMenu?.addEventListener('click', (e) => {
    ...
    if (item.dataset.action === 'insert-manually') {
      insertFromPlacementBar('manual');
    } else if (item.dataset.action === 'insert-all-occurrences') {
      insertFromPlacementBar('all');
    } else if (item.dataset.action === 'insert-selected-occurrences') {
      insertFromPlacementBar('selected');
    }
  });

  els.placementBarSettingsBtn?.addEventListener('click', (e) => {
    e.stopPropagation();
    closePlacementBarInsertMenu();
    const markerNum = getActiveBarMarker()?.num ?? getFocusedPlacementMarkerNum();
    openPlacementSettingsPopover(markerNum);
  });
...
function updateSuggestionsPopover() {
  ...
  els.suggestionsPopover.hidden = !shouldShowPlacementGuides()
    || shouldShowPlacementBar()
    || state.placementSettingsOpen;
}
...
function getPlacementSettingsBadgeConfig(markerNum) {
  return PLACEMENT_SETTINGS_BADGES[markerNum] ?? PLACEMENT_SETTINGS_BADGES[1];
}

function updatePlacementSettingsBadge() {
  if (!els.placementSettingsBadge) return;

  const markerNum = getPlacementSettingsMarkerNum();
  const config = getPlacementSettingsBadgeConfig(markerNum);

  els.placementSettingsBadge.textContent = config.text;
  els.placementSettingsBadge.classList.remove(...PLACEMENT_SETTINGS_BADGE_VARIANTS);
  els.placementSettingsBadge.classList.add(`placement-settings-popover__badge--${config.variant}`);
  els.placementSettingsBadge.hidden = false;

  if (els.placementSettingsWarning) {
    const warningText = config.warning ?? '';
    els.placementSettingsWarning.textContent = warningText;
    els.placementSettingsWarning.hidden = !warningText;
  }

  if (els.placementSettingsPopover) {
    els.placementSettingsPopover.classList.toggle(
      'placement-settings-popover--has-warning',
      Boolean(config.warning),
    );
  }
}
...
function updatePlacementSettingsPopover() {
  ...
  if (els.placementSettingsPageLabel) {
    els.placementSettingsPageLabel.textContent = `${current} of ${total}`;
  }
  if (els.placementSettingsPrev) {
    els.placementSettingsPrev.classList.toggle('is-disabled', state.placementBarIndex <= 0);
  }
  if (els.placementSettingsNext) {
    els.placementSettingsNext.classList.toggle('is-disabled', state.placementBarIndex >= total - 1);
  }

  if (els.placementSettingsBadge) {
    updatePlacementSettingsBadge();
  }
...
function getComponentOpacity(component) {
  if (isPlacingComponent2()
    && (component.fileId === COMPONENT_1_ID || component.fileId === COMPONENT_2_ID)) {
...
function renderMarkerHoverToolbarHtml(marker) {
  const toolbarStyle = getMarkerToolbarStyle(marker);
  const settingsHtml = `
...
function renderComponent1Overlay() {
  ...
  previewHtml = previewMarkerNums
    ...
    .filter(Boolean)
    .map((marker) => renderPreviewHtml(marker, overlay))
    .join('');
  ...
  const markersHtml = markers
    ...
    .map((marker) => {
      ...
      let markerClass = '';
      if (marker.num === highlightMarkerNum) {
...
function updatePlacingGhost() {
  ...
  const markerNums = shouldShowPlacementGuides() ? getPlacingGhostMarkerNums() : [];
  trimPlacingGhosts(markerNums.length + 1);
  ...
  markerNums.forEach((markerNum, index) => {
    ...
    if (ghost) {
      ghost.dataset.markerNum = String(markerNum);
      applyPlacingGhostLayout(ghost, layout, true);
    }
  });
  ...
  const cursorGhost = ensurePlacingGhost(markerNums.length);
  if (!cursorGhost) return;

  delete cursorGhost.dataset.markerNum;
  applyPlacingGhostLayout(cursorGhost, layout, false);
}
...
function bindOverlayInteractions() {
  ...
  els.canvasComponents.addEventListener('mouseout', (e) => {
    ...
    if (state.selectedMarkerNums.length > 0) {
      ...
      state.hoveredMarker = getActiveBarMarker()?.num ?? null;
    }
    refreshPlacementOverlay();
    updatePlacingGhost();
  });
...
const PROTOTYPE_NAV_HEIGHT = 36;
const PROTOTYPE_FRAME_HEIGHT = ARTBOARD_HEIGHT + PROTOTYPE_NAV_HEIGHT;

function fitArtboard() {
  const frame = els.prototypeFrame ?? document.querySelector('.prototype-frame');
  if (!frame) return;
  const scale = Math.min(
    window.innerWidth / ARTBOARD_WIDTH,
    window.innerHeight / PROTOTYPE_FRAME_HEIGHT,
  );
