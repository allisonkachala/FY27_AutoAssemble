...
function getHoveredPreviewCanvasRect() {
  ...
  const component1 = state.canvasComponents.find((c) => c.fileId === COMPONENT_1_ID);
  const marker = COMPONENT_1_OVERLAY.markers.find((m) => m.num === state.hoveredMarker);
...
function renderMarkerButton(marker, style, hoveredClass = '') {
  return `<button type="button" class="component-marker component-marker--${marker.variant}${hoveredClass}" data-marker="${marker.num}" style="${style}" aria-label="Placement option ${marker.num}"><span class="component-marker__header"><span class="component-marker__label">${marker.num}</span></span></button>`;
}
...
function renderComponent1Overlay() {
  ...
  const hoveredMarker = markers.find((marker) => marker.num === state.hoveredMarker);

  let previewHtml = '';
  let hoverGroupHtml = '';
  if (hoveredMarker) {
    const preview = getPreviewPosition(hoveredMarker);
    const previewStyle = [
    ...
    previewHtml = `<img class="placement-preview" src="${PREVIEW_IMAGES[hoveredMarker.variant]}" alt="" style="${previewStyle}" />`;
    hoverGroupHtml = renderHoverGroupHtml(hoveredMarker, overlay);
  }

  const markersHtml = markers
    .filter((marker) => marker.num !== state.hoveredMarker)
    .map((marker) => {
      const style = [
      ...
      return renderMarkerButton(marker, style);
    }).join('');
...
function ensurePlacingGhost() {
  const file = state.placement.active ? getFileById(state.placement.fileId) : null;
  if (!file?.insertPreview) return null;

  let ghost = els.canvasComponents.querySelector('.canvas-component.is-placing');
  if (!ghost) {
...
function updatePlacingGhost() {
  ...
  const ghost = ensurePlacingGhost();
  if (!ghost) return;
